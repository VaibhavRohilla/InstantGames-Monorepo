...
export class RouletteService {
  ...
  async placeBet(ctx: AuthContext, dto: RouletteBetDto, idempotencyKey: string): Promise<RouletteBetResponse> {
    ...
    const result = await this.gameBetRunner.run({
      ...
      request: {
        betAmount,
        payload: { selection: dto.selection },
  ...
  private toResponse(dto: RouletteBetDto, currency: string, result: GameBetRunnerResult): RouletteBetResponse {
    return {
