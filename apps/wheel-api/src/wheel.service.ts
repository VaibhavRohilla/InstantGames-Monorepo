...
export class WheelService {
  ...
  async placeBet(ctx: AuthContext, dto: WheelBetDto, idempotencyKey: string): Promise<WheelBetResponse> {
    ...
    const result = await this.gameBetRunner.run({
      ...
      request: {
        betAmount,
        payload: { segmentGuess: dto.segmentGuess },
  ...
  private toResponse(dto: WheelBetDto, currency: string, result: GameBetRunnerResult): WheelBetResponse {
    return {
